...
export default function Tags() {
  const { data, error, ref } = useAdmin('/api/tags');
...
function TableHeadBody({
  ...
  return (
    <React.Fragment>
      ...
      <tbody>
        {tools.map((tool) => {
          if (tool?.type == 'Tag' && tool.attributes !== undefined)
            return (
              <tr key={tool.id}>
  ...
  */
  ...
  const onSubmitModalAdd = reactForm.handleSubmit(async (data) => {
    try {
      ...
      const request = await fetcherGeneric<DocMeta>('/api/tags', {
        method: 'post',
        body: JSON.stringify({
          data: {
            type: 'Tool',
      ...
      clientHandlerError(err, dispatch, mutate, '/api/tags').catch((err) =>
        console.error(err),
      );
    }
  });
